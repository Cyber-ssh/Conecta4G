...
function inst_db { 
sed -i "s;dominio;$IP;g" /var/www/html/conecta4g.sql > /dev/null 2>&1
sleep 1
if [[ -e "/var/www/html/conecta4g.sql" ]]; then
    mysql -h localhost -u root -p"$pwdroot" --default_character_set utf8 conecta4g < /var/www/html/conecta4g.sql > /dev/null 2>&1
    rm /var/www/html/conecta4g.sql > /dev/null 2>&1
else
    clear
...
echo ""
echo -e "                 \033[1;36mBy @swittecnologia\033[1;36m"
echo ""
